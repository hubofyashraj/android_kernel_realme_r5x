 ...
 */
unsigned int sysctl_sched_latency __read_mostly = 16000000ULL;
 ...
 */
unsigned int sysctl_sched_tunable_scaling __read_mostly;
 ...
 */
const static unsigned int sched_nr_latency = 8;
...
#else
static inline int select_idle_sibling(struct task_struct *p, int prev, int target)
#endif
{
	struct sched_domain *sd;
...
static inline bool task_fits_capacity(struct task_struct *p,
					long capacity)
{
	return capacity > boosted_task_util(p);
}
				   ...
				   struct find_best_target_env *fbt_env)
{
	...
	*backup_cpu = -1;
	...
	do {
		 ...
		 * break from searching more groups based on the stune.boosted and
...
{
	return capacity_orig_of(cpu) < (cpu_util(cpu) + delta);
}
		    ...
		    int sibling_count_hint)
{
	...
	if (sd_flag & SD_BALANCE_WAKE) {
		...
		if (sync && _cpus_allowed && !_wake_cap &&
...
pick_cpu:
		...
		if (energy_sd)
			new_cpu = find_energy_efficient_cpu(energy_sd, p, cpu, prev_cpu,
								sync);
...
int can_migrate_task(struct task_struct *p, struct lb_env *env)
{
	...
	if (sched_feat(ENERGY_AWARE) && !sd_overutilized(env->sd) &&
...
static inline void update_sd_lb_stats(struct lb_env *env, struct sd_lb_stats *sds)
{
	...
	do {
		...
		update_sg_lb_stats(env, sg, sgs, &overload, &overutilized,
						&misfit_task);
...
next_group:
	 ...
	 */
	if (lb_sd_parent(env->sd) &&
	    sds->total_capacity < sds->total_util)
...
more_balance:
		...
		if (busiest->cfs.h_nr_running <= 1 ||
		    (busiest->active_balance && busiest->cfs.h_nr_running <= 2)) {
...
static inline bool nohz_kick_needed(struct rq *rq, bool only_update)
{
	...
	*/
	 ...
	 */
	if (rq->nr_running >= 2 &&
	    (!sched_feat(ENERGY_AWARE) || cpu_overutilized(cpu)))
		return true;
	...
	rcu_read_lock();
	sds = rcu_dereference(per_cpu(sd_llc_shared, cpu));
	if (sds && !sched_feat(ENERGY_AWARE)) {
		/*
