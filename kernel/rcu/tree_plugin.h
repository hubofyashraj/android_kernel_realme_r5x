 ...
 * The string after the "rcunocbs=" is either "all" for all CPUs, or a
...
static int __init rcu_nocb_setup(char *str)
{
	...
	if (!strcasecmp(str, "all"))
		...
		if (cpulist_parse(str, rcu_nocb_mask)) {
			pr_warn("rcunocbs= bad CPU range, all CPUs set\n");
			cpumask_setall(rcu_nocb_mask);
		}
	return 1;
}
__setup("rcunocbs=", rcu_nocb_setup);
...
void __init rcu_init_nohz(void)
{
	...
	if (!cpumask_subset(rcu_nocb_mask, cpu_possible_mask)) {
		pr_info("\tNote: kernel parameter 'rcunocbs=', 'nohz_full', or 'isolcpus=' contains nonexistent CPUs.\n");
...
static void rcu_spawn_one_nocb_kthread(int cpu)
{
	...
	t = kthread_run(rcu_nocb_cb_kthread, rdp, "rcuo%c/%d", rcu_state.abbr, cpu);
