...
static int perf_cpu_fourth_index = -1;
static int lp_cpu_index = -1;
static int lp_cpu_first_index = -1;
static int lp_cpu_second_index = -1;
static int lp_cpu_third_index = -1;
static int lp_cpu_fourth_index = -1;
...
static void affine_one_perf_thread(struct irqaction *action)
{
	...
	if (action->flags & IRQF_PERF_FOURTH_AFFINE)
		mask = cpu_perf_fourth_mask;

	if (action->flags & IRQF_LP_AFFINE)
		mask = cpu_lp_mask;

	if (action->flags & IRQF_LP_FIRST_AFFINE)
		mask = cpu_lp_first_mask;

	if (action->flags & IRQF_LP_SECOND_AFFINE)
		mask = cpu_lp_second_mask;

	if (action->flags & IRQF_LP_THIRD_AFFINE)
		mask = cpu_lp_third_mask;

	if (action->flags & IRQF_LP_FOURTH_AFFINE)
		mask = cpu_lp_fourth_mask;
...
static void affine_one_perf_irq(struct irq_desc *desc, unsigned int perf_flag)
{
	...
	if (perf_flag & IRQF_PERF_FOURTH_AFFINE) {
		mask = cpu_perf_fourth_mask;
		mask_index = &perf_cpu_fourth_index;
	}

	if (perf_flag & IRQF_LP_AFFINE) {
		mask = cpu_lp_mask;
		mask_index = &lp_cpu_index;
	}

	if (perf_flag & IRQF_LP_FIRST_AFFINE) {
		mask = cpu_lp_first_mask;
		mask_index = &lp_cpu_first_index;
	}

	if (perf_flag & IRQF_LP_SECOND_AFFINE) {
		mask = cpu_lp_second_mask;
		mask_index = &lp_cpu_second_index;
	}

	if (perf_flag & IRQF_LP_THIRD_AFFINE) {
		mask = cpu_lp_third_mask;
		mask_index = &lp_cpu_third_index;
	}

	if (perf_flag & IRQF_LP_FOURTH_AFFINE) {
		mask = cpu_lp_fourth_mask;
		mask_index = &lp_cpu_fourth_index;
	}
...
void reaffine_perf_irqs(bool from_hotplug)
{
	...
	if (!from_hotplug || !perf_crit_suspended) {
		...
		perf_cpu_fourth_index = -1;
		lp_cpu_index = -1;
		lp_cpu_first_index = -1;
		lp_cpu_second_index = -1;
		lp_cpu_third_index = -1;
		lp_cpu_fourth_index = -1;
		list_for_each_entry(data, &perf_crit_irqs, list) {
			struct irq_desc *desc = data->desc;
...
__setup_irq(unsigned int irq, struct irq_desc *desc, struct irqaction *new)
{
	...
	if (!shared) {
		...
		if (new->flags & (
				IRQF_PERF_AFFINE |
				IRQF_PERF_FIRST_AFFINE |
				IRQF_PERF_SECOND_AFFINE |
				IRQF_PERF_THIRD_AFFINE |
				IRQF_PERF_FOURTH_AFFINE |
				IRQF_LP_AFFINE |
				IRQF_LP_FIRST_AFFINE |
				IRQF_LP_SECOND_AFFINE |
				IRQF_LP_THIRD_AFFINE |
				IRQF_LP_FOURTH_AFFINE)) {
