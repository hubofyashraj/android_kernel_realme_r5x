...
static struct df_boost_drv df_boost_drv_g __read_mostly = {
	BOOST_DEV_INIT(df_boost_drv_g, DEVFREQ_MSM_CPU_BW,
		       CONFIG_DEVFREQ_MSM_BOOST_FREQ),
	BOOST_DEV_INIT(df_boost_drv_g, DEVFREQ_MSM_GPU_BW,
		       CONFIG_DEVFREQ_MSM_BOOST_FREQ)
};
...
static int __init devfreq_boost_init(void)
{
	...
	for (i = 0; i < DEVFREQ_MAX; i++) {
		...
		thread[i] = kthread_run_perf_critical(cpu_perf_second_mask,
